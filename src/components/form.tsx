...
const Form = () => {
  ...
  return (
    <div className="bg-white/40 px-5 py-7 backdrop-blur-sm">
      <form onSubmit={handleSubmit(onSubmit)} className="w-full">
        <fieldset disabled={isSubmitting} className="flex flex-col gap-4 w-full min-w-0 border-0 p-0 m-0">
          <div className="pb-6 w-full relative">
            <label>
              <p className="text-2xl mb-2">Ваше имя:</p>
              <input className="px-3 py-3.5 text-2xl bg-white/50  hover:bg-white/70 w-full focus:bg-white/80 transition-colors duration-300 ease-in-out" autoComplete="off" {...register("name")} />
            </label>
            {errors.name?.message && <span className="absolute left-0 bottom-0 text-red-700">Введите имя</span>}
          </div>
          <div className="pb-6 w-full relative">
            <label>
              <p className="text-xl mb-2">Комментарий:</p>
              <textarea className="px-3 py-3.5 text-2xl bg-white/50  hover:bg-white/70 w-full focus:bg-white/80 resize-y min-h-25 w-full transition-colors duration-300 ease-in-out" {...register("text")} />
            </label>
